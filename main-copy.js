...
function init() {
  ...
  pinksoldier.load(
    "assets/models/squid_game_pinksoldier/scene.gltf",
    (gltf) => {
  ...
  pinksoldier_triangle1.load(
    "assets/models/squid_game_pinksoldier_triangle/scene.gltf",
    (gltf) => {
      gltf.scene.traverse(function (node) {
        if (node.isMesh) {
          node.castShadow = true;
...
function lookBackward() {
  ...
  gsap.to(doll.position, { duration: 0.45, z: -25 });
  soundBack.play();
...
function lookForward() {
  ...
  gsap.to(doll.position, { duration: 0.45, z: 0 });
  soundFront.play();
