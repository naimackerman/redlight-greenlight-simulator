...
class Player {
  ...
  w_run() {
    ...
    this.playerInfo.w_velocity = 0.03;
    this.w = true;
    this.action.play();
  }
  s_run() {
    if (this.playerInfo.isDead) return;
    this.playerInfo.s_velocity = 0.03;
    this.s = true;
    this.action.play();
  }
  a_run() {
    if (this.playerInfo.isDead) return;
    this.playerInfo.a_velocity = 0.03;
    this.a = true;
    this.action.play();
  }
  d_run() {
    if (this.playerInfo.isDead) return;
    this.playerInfo.d_velocity = 0.03;
    this.d = true;
    this.action.play();
  }

  w_stop() {
    gsap.to(this.playerInfo, { duration: 0.1, w_velocity: 0 });
    this.w = false;
  ...
  a_stop() {
    gsap.to(this.playerInfo, { duration: 0.1, a_velocity: 0 });
    this.a = false;
    if (this.w || this.a || this.s || this.d) {
    } else {
      this.action.stop();
  ...
  s_stop() {
    gsap.to(this.playerInfo, { duration: 0.1, s_velocity: 0 });
    this.s = false;
    if (this.w || this.a || this.s || this.d) {
    } else {
      this.action.stop();
  ...
  d_stop() {
    gsap.to(this.playerInfo, { duration: 0.1, d_velocity: 0 });
    this.d = false;
    if (this.w || this.a || this.s || this.d) {
    } else {
      this.action.stop();
  ...
  check(world) {
    ...
    var velocity = false;
    if (
      this.playerInfo.w_velocity > 0 ||
      this.playerInfo.a_velocity > 0 ||
      this.playerInfo.s_velocity > 0 ||
      this.playerInfo.d_velocity > 0
    ) {
      velocity = true;
    }
  ...
  update(world) {
    this.check(world);
    if (this.playerObj.position.x < -9) {
      // left wall
      this.playerObj.position.x += 0.001;
    } else if (this.playerObj.position.x > 9) {
      // right wall
      this.playerObj.position.x -= 0.001;
    } else if (this.playerObj.position.z > 8) {
      // back wall
  ...
  async loadPlayer(radius = 0.25, posY = 0, color = 0xffffff) {
    const loader = new GLTFLoader();

    const playerData = await Promise.all([
      loader.loadAsync("./assets/models/player/scene_jogging.glb"),
    ]);
