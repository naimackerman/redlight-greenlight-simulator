...
function onStartSkybox() {
  ...
  fbxLoader.load(
    "assets/models/mountain-terrain/source/Mountain_Final_fbx_.fbx",
    (model) => {
...
function onStartSoldier() {
  fbxLoader.load(
    "assets/models/squid-game-guards/source/guards.fbx",
    (model) => {
      model.name = "Soldier1";
...
function onStartFloor() {
  ...
  //   new THREE.ConeGeometry(2, 10, 20),
  //   new THREE.MeshPhongMaterial({ color: 0xdbde40 })
  // );
  // cone.position.set(-20, 2.5, -30);
  // cone.receiveShadow = true;
  // cone.castShadow = true;
  // scene.add(cone);

  // const cylinder = new THREE.Mesh(
  //   new THREE.CylinderGeometry(1, 20, 50, 50),
  //   new THREE.MeshPhongMaterial({ color: 0x3ea34c })
  // );
  // cylinder.position.set(-15, 3, -30);
...
function onStartDoll() {
  fbxLoader.load(
    "assets/models/squid-game-giant-doll/source/squidGame_Doll.fbx",
    (model) => {
      model.name = "Doll";
...
function onUpdate(dt) {
  if (isStart == 1) {
    ...
    camera.lookAt(
      player.handler.position.x,
      player.handler.position.y + 5,
      player.handler.position.z
    );
  }
}
